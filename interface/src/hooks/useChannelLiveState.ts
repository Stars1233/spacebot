...
export function isOpenCodeWorker(worker: {
  workerType?: string;
  task?: string;
}): boolean {
  return (
    worker.workerType === "opencode" ||
    (worker.task?.startsWith("[opencode]") ?? false)
  );
}
...
export interface ChannelLiveState {
  ...
  compaction: ActiveCompaction | null;
  streamingMessageId: string | null;
  /** Current turn's aggregated model reasoning (thinking), if any. */
...
export function useChannelLiveState(channels: ChannelInfo[]) {
        ...
        setLiveStates((prev) => {
          ...
          for (const [channelId, snapshot] of Object.entries(statusMap)) {
            ...
            next[channelId] = {
              ...
              workers,
              branches,
              compaction: snapshot.active_compaction
                ? {
                    kind: snapshot.active_compaction.kind,
                    startedAt: new Date(
                      snapshot.active_compaction.started_at,
                    ).getTime(),
                  }
                : null,
            };
  ...
  const handleTypingState = useCallback((data: unknown) => {
    ...
    setLiveStates((prev) => {
      const existing = getOrCreate(prev, event.channel_id);
      // A new turn starting (typing true) means any reasoning from the
      // previous turn is stale — clear it so the current turn's thinking
      // renders fresh.
      return {
        ...prev,
        [event.channel_id]: {
          ...
          thinking: event.is_typing ? null : existing.thinking,
        },
      };
    });
  }, []);

  const handleReasoningDelta = useCallback((data: unknown) => {
    const event = data as ReasoningDeltaEvent;
    setLiveStates((prev) => {
      const existing = getOrCreate(prev, event.channel_id);
      return {
        ...prev,
        [event.channel_id]: { ...existing, thinking: event.aggregated_reasoning },
      };
    });
  }, []);

  const handleCompactionStarted = useCallback((data: unknown) => {
    const event = data as CompactionStartedEvent;
    setLiveStates((prev) => {
      const existing = getOrCreate(prev, event.channel_id);
      return {
        ...prev,
        [event.channel_id]: {
          ...existing,
          compaction: { kind: event.kind, startedAt: Date.now() },
        },
      };
    });
  }, []);

  const handleCompactionCompleted = useCallback((data: unknown) => {
    const event = data as CompactionCompletedEvent;
    setLiveStates((prev) => {
      const existing = prev[event.channel_id];
      if (!existing) return prev;
      return { ...prev, [event.channel_id]: { ...existing, compaction: null } };
    });
  }, []);
    ...
    setLiveStates((prev) => {
      ...
      if (
        existing.timeline.some(
          (item) =>
            item.type === "checkpoint" && item.id === event.checkpoint_id,
        )
      ) {
        return prev;
      }
        ...
        api
          ...
          .then((data) => {
            setLiveStates((current) => {
              ...
              const olderItems = data.items.filter(
